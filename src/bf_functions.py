    ...
    def write_int(self, device_name, data, offset=0, frequencies=all, fft_bins=[], blindwrite=False):
        ...
        if len(fpgas) != 0:
            if self.config.simulate == True:
                ...
                nottimedout, rv = corr.corr_functions.non_blocking_request\
                    (fpgas = fpgas, timeout = timeout, request = 'write', request_args = [name, offset*4, datum])
...
#   CALIBRATION
    ...
    def cal_spectrum_get(self, beam, ant_str, from_fpga=True):
        ...
        if from_fpga:
            ...
            fpga_values = self.bf_read_int(beam=beam, destination='calibrate', offset=0,
                                           antennas=[ant_str], frequencies=all)
        else:
...
#        spead stuff that does not care about beam
        ...
        spead_ig.add_item(name="feng_pkt_len",id=0x1021,
            description="Payload size of 10GbE packet exchange between F and X engines in 64 bit words. Usually equal "
                        "to the number of spectra accumulated inside X engine. F-engine correlator internals.",
            shape=[],fmt=spead.mkfmt(('u',spead.ADDRSIZE)),
...
#TODO ADD VERSION INFO!
        ...
        spead_ig.add_item(name="ddc_mix_freq",id=0x1043,
            description="Digital downconverter mixing freqency as a fraction of the ADC sampling "
                        "frequency. eg: 0.25. Set to zero if no DDC is present.",
        ...
        spead_ig.add_item(name="beng_out_bits_per_sample",id=0x1050,
            description="The number of bits per value in the beng output. Note that this is for a single "
                        "value, not the combined complex value size.",
            shape=[],fmt=spead.mkfmt(('u',spead.ADDRSIZE)),
        ...
        spead_ig.add_item(name=('timestamp'), id=0x1600,
            description='Timestamp of start of this block of data. uint counting multiples of ADC samples since '
                        'last sync (sync_time, id=0x1027). Divide this number by timestamp_scale (id=0x1046) to '
                        'get back to seconds since last sync when this block of data started.',
    ...
    def spead_passband_meta_issue(self, beams=all):
        ...
        for beam in beams:
            ...
            spead_ig.add_item(name=beam, id=beam_data_id,description="Raw data for bengines in the system.  "
                                                                     ...
                                                                     "of values in time order where the number of samples "
                                                                     "in a block is given by xeng_acc_len (id 0x101F)."
    ...
    def spead_time_meta_issue(self, beams=all):
        ...
        spead_ig.add_item(name='sync_time',id=0x1027,
            description="Time at which the system was last synchronised (armed and triggered by a 1PPS) "
                        "in seconds since the Unix Epoch.",
            shape=[],fmt=spead.mkfmt(('u',spead.ADDRSIZE)),
            init_val=val)
        #scale factor for timestamp
        spead_ig.add_item(name="scale_factor_timestamp",id=0x1046,
            description="Timestamp scaling factor. Divide the SPEAD data packet timestamp by this number "
    ...
    def spead_eq_meta_issue(self, beams=all):
        ...
        for in_n,ant_str in enumerate(self.c.config._get_ant_mapping_list()):
            ...
            spead_ig.add_item(name="eq_coef_%s"%(ant_str),id=0x1400+in_n,
                description="The unitless per-channel digital scaling factors implemented prior "
                            "to requantisation, post-FFT, for input %s. Complex number real,imag 32 bit integers."%(ant_str),
            ...
            for in_n,ant_str in enumerate(self.ants2ants(beam,all)):
                ...
                ig.add_item(name="beamweight_input%s"%(ant_str),id=0x2000+in_n,
                    description="The unitless per-channel digital scaling factors implemented "
                                "prior to combining antenna signals during beamforming for input %s. Complex number "
